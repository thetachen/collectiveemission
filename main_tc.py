...
for it in range(Ntimes):
    ...
    if it%Nskip==0:
        times.append( it*dt )
        Pmol1.append( np.linalg.norm(model1.Cj[model1.Imol:model1.Imol+model1.Nmol])**2 )
        Pmol2.append( np.linalg.norm(model2.Cj[model2.Imol:model2.Imol+model2.Nmol])**2 )

        IPR1.append( np.linalg.norm(model1.Cj[model1.Imol:model1.Imol+model1.Nmol])**4 / np.sum(np.abs(model1.Cj[model1.Imol:model1.Imol+model1.Nmol])**4) )
        IPR2.append( np.linalg.norm(model2.Cj[model2.Imol:model2.Imol+model2.Nmol])**4 / np.sum(np.abs(model2.Cj[model2.Imol:model2.Imol+model2.Nmol])**4) )
        ...
        print("{t}\t{d}\t{dP}\t{dE}".format(t=it*dt,d=model1.getDisplacement(), 
                                                    dP=np.linalg.norm(model1.Cj[model1.Imol:model1.Imol+model1.Nmol])**2-1.0, 
